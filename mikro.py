...
# ------------------------------------------
    ...
    with tab_step1:
      ...
      st.latex(
          rf"\mathcal{{L}}(X, Y, \lambda) = (X \cdot Y) + \lambda ({m_anggaran}"
          rf" - {px_makanan}X - {py_data}Y)"
      )
      st.info(
    ...
    with tab_step2:
      ...
      with col_foc1:
        ...
        st.caption(
            f"Pada titik optimal: $MU_x = {mu_x_opt:.2f}$ util dari porsi ke-{opt_x:.1f} Makanan."
        )
      ...
      with col_foc2:
        ...
        st.caption(
            f"Pada titik optimal: $MU_y = {mu_y_opt:.2f}$ util dari unit ke-{opt_y:.1f} Data."
        )
